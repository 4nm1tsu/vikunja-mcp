...
import { registerBucketTool } from './bucket';
...
export function registerTools(
  ...
  // Kanban bucket moves (column/status). Uses the session token directly (the bundled
  // node-vikunja client does not expose the views-based bucket endpoints), so it does
  // not depend on clientFactory and is always registered.
  registerBucketTool(server, authManager);
